...
if(isset($_POST['submited2'])){
		  ...
		  $sql="SELECT * FROM tbl_petients WHERE Firstname='$fname' && Sirname='$sname' && DOB='$dob'";
                   $resultn=mysqli_query($db,$sql);                    
                         if($rowcount=mysqli_num_rows($resultn)==0)
                         {                                           						
							 $sqln = "INSERT INTO tbl_petients (Mtitle,Firstname,Middlename,Sirname,Gender,Phone,NextKphone,DOB,Location,Guardian,Relation,Date,Payment)
			               VALUES ('$mtitle','$fname','$mname','$sname','$gender','$phone','$kphone','$dob','$village','$guardian','$relationship','$date','$patientpay')";
		                     $db->query($sqln);
